...
const protect = async (req, res, next) => {
  try {
    ...
    if (apiKey) {
      ...
      db.execute({ sql: "UPDATE api_keys SET last_used_at = NOW()::text WHERE id = ?", args: [keyRow.rows[0].id] }).catch(()=>{});
      return next();
    }
    ...
    db.execute({ sql: "UPDATE users SET last_seen = NOW()::text WHERE id = ?", args: [decoded.id] }).catch(()=>{});
    next();
  } catch (err) {
