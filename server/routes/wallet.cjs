...
router.post('/withdraw', protect, async (req, res) => {
  try {
    ...
    // Atomic deduct: the WHERE clause makes this the actual balance check —
    // if two withdrawal requests race in at once, only one can succeed here.
    const deducted = await db.execute({
      sql: 'UPDATE users SET wallet_balance = wallet_balance - ? WHERE id = ? AND wallet_balance >= ?',
      args: [amt, req.user.id, amt]
    });
