...
const { sendOTP, sendVerification } = require('../utils/email.cjs');
...
router.post('/register', async (req, res) => {
  try {
    ...
    if (username.length < 3 || username.length > 32 || !/^[a-zA-Z0-9_.]+$/.test(username))
      return res.status(400).json({ message: 'Username must be 3-32 characters (letters, numbers, . and _ only)' });
