...
const { getEffectivePlan } = require('../utils/plans.cjs');
...
const TOOL_PROMPTS = {
  chat: 'You are ScottyAI, a friendly and helpful assistant created by Scotty for ScottyHub — a digital income and WhatsApp bot platform from Zimbabwe. Help users with WhatsApp bots using Baileys.js, JavaScript, Node.js, deploying on Render/pxxl, making money online, and ScottyHub features. Always respond in English only. Never say you are DeepSeek or any other AI — you are ScottyAI. Be concise, friendly, and practical. Keep responses short and mobile-friendly.',
...
router.get('/credits', protect, async (req, res) => {
  ...
  res.json({ used, limit: plan.aiDailyLimit, remaining: Math.max(0, plan.aiDailyLimit - used), plan: plan.id });
});

// POST /api/ai/chat — original ScottyAI multi-turn chatbot (kept for the AI Chat tab)
router.post('/chat', protect, async (req, res) => {
  ...
  try {
    ...
    if (!credit.allowed) {
      return res.status(429).json({ message: `Daily AI limit reached (${credit.limit}/day on your plan). Try again tomorrow or upgrade to Premium.` });
    }
