...
const { sendPushToUser } = require('../utils/push.cjs');
const { onlineUsers } = require('../socket/presence.cjs');
...
async function createRoomMessage(room, senderId, content, type = 'text') {
  ...
  // Group chats are small enough that a push per new message is welcome
  // (unlike 'community'/'feed' which would spam every user on the site).
  // Only offline members get one — anyone with the app open already sees it live.
  if (room.startsWith('group:')) {
    const groupId = room.slice('group:'.length);
    db.execute({ sql: 'SELECT user_id FROM group_members WHERE group_id=? AND user_id != ?', args: [groupId, senderId] })
      .then(({ rows }) => {
        rows.forEach(m => {
          if (!onlineUsers.has(m.user_id)) {
            sendPushToUser(m.user_id, {
              title: row.rows[0].sender_name || 'New group message',
