  ...
  try {
    ...
    const posts = await db.execute({
      sql: `SELECT p.id, p.content, p.media_url, p.media_type, p.pinned, p.created_at,
            ...
            WHERE p.channel = ? AND (
              p.author_id IN (SELECT following_id FROM follows WHERE follower_id = ?)
              OR p.author_id = ? OR p.pinned = 1
            )
            ORDER BY p.pinned DESC, p.created_at DESC
            LIMIT ? OFFSET ?`,
      args: [channel, req.user.id, req.user.id, limit, offset]
    });
    ...
    res.json(posts.rows.map(p => ({
      id: p.id, content: p.content, mediaUrl: p.media_url, mediaType: p.media_type || '', pinned: p.pinned,
