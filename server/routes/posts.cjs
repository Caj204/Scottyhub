...
const { progressMission } = require('../utils/missions.cjs');

// GET /api/posts/public/:id — minimal public post data, no auth required.
// Used for shared-link previews (see server.js /post/:id) and the in-app
// shared-link modal, so a link works even before the visitor logs in.
router.get('/public/:id', async (req, res) => {
  try {
    const result = await db.execute({
      sql: `SELECT p.id, p.content, p.media_url, p.media_type, p.created_at,
...
router.get('/', protect, async (req, res) => {
  try {
    ...
    // channel: 'community' (default, the discussion feed) or 'blog' (TikTok-style media feed)
    const channel = req.query.channel === 'blog' ? 'blog' : 'community';

    const args = [channel];
    let where = `p.channel = ? AND ${groupId ? 'p.group_id = ?' : 'p.group_id IS NULL'}`;
    if (groupId) args.push(groupId);
    args.push(limit, offset);

    const posts = await db.execute({
      sql: `SELECT p.id, p.content, p.media_url, p.media_type, p.created_at, p.group_id, p.poll_options,
                   u.id as author_id, u.username as author_username, u.avatar as author_avatar,
                   u.plan as author_plan, u.plan_expires_at as author_plan_expires_at,
                   u.is_verified_badge as author_verified_badge,
    ...
    const result = posts.rows.map(p => {
      ...
      const planActive = p.author_plan_expires_at && new Date(p.author_plan_expires_at) > new Date();
      return {
        id: p.id,
        content: p.content,
        mediaUrl: p.media_url,
        ...
        poll,
        author: {
          id: p.author_id, username: p.author_username, avatar: p.author_avatar,
          plan: planActive ? p.author_plan : 'free',
          verified: !!p.author_verified_badge
        },
...
router.post('/', protect, async (req, res) => {
  try {
    const { content, mediaUrl, mediaType, groupId, pollOptions, channel } = req.body;
    const postChannel = channel === 'blog' ? 'blog' : 'community';
    // Blog posts are media-first (TikTok-style), so allow an empty caption if media is attached
    if (!content && !(postChannel === 'blog' && mediaUrl)) {
    ...
    const id = uuidv4();
    await db.execute({
      sql: 'INSERT INTO posts (id, author_id, content, media_url, media_type, channel, group_id, poll_options) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      args: [id, req.user.id, content || '', mediaUrl || '', mediaType || '', postChannel, groupId || null, pollData]
    });
