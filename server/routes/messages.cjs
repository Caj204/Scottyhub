...
router.post('/:convId', async (req, res) => {
  try {
    ...
    if (io) {
      ...
      // Recipient isn't connected right now (app closed/backgrounded) — fire a
      // real OS-level push so they still find out about the message.
      if (!onlineUsers.has(otherId)) {
        sendPushToUser(otherId, {
          title: `${msg.rows[0].sender_name || 'Someone'} sent you a message`,
...
router.put('/last-seen', async (req, res) => {
  try {
    await db.execute({
      sql: "UPDATE users SET last_seen=NOW()::text WHERE id=?",
      args: [req.user.id]
    });
    res.json({ ok: true });
  } catch (err) {
