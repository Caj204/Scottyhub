...
router.get('/', protect, async (req, res) => {
  try {
    ...
    const { periodKeyFor } = require('../utils/missions.cjs');
...
router.post('/checkin', protect, async (req, res) => {
  try {
    ...
    if (last) {
      ...
      newStreak = (last === yesterdayStr) ? (req.user.streak_count || 0) + 1 : 1;
    }
    ...
    await db.execute({
      sql: `UPDATE users SET streak_count = ?, last_checkin = NOW()::text,
                              points_balance = points_balance + ?, xp = xp + ?
            WHERE id = ?`,
      args: [newStreak, pointsReward, xpReward, uid]
    });
    ...
    const awardBadge = async (badgeId) => {
      await db.execute({
        sql: 'INSERT INTO user_badges (user_id, badge_id) VALUES (?, ?) ON CONFLICT (user_id, badge_id) DO NOTHING',
        args: [uid, badgeId]
      }).catch(() => {});
    };
    ...
    await db.execute({
      ...
      args: [
        ...
        'Daily Check-in', `You earned ${pointsReward} points! Streak: ${newStreak} days`, 'reward'
      ]
    });

    res.json({ message: 'Checked in!', streak: newStreak, pointsEarned: pointsReward, xpEarned: xpReward, newLevel });
  } catch (err) {
    console.error(err);
