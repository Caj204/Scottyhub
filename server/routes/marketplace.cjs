...
const PLATFORM_COMMISSION = 0.15; // ScottyHub keeps 15% of every sale/rental
...
router.post('/listings', protect, async (req, res) => {
  try {
    let { name, description, category, coverImage, downloadUrl, demoUrl, priceCOPS, rentalPricePerDay } = req.body;
    ...
    const price = parseInt(priceCOPS, 10);
    if (!Number.isFinite(price) || price <= 0) {
      return res.status(400).json({ message: 'priceCOPS must be a positive number' });
    }
    let rentalPrice = parseInt(rentalPricePerDay, 10);
    if (!Number.isFinite(rentalPrice) || rentalPrice < 0) rentalPrice = 0;
    ...
    await db.execute({
      sql: `INSERT INTO bot_listings (id, seller_id, name, description, category, cover_image, download_url, demo_url, price_cops, rental_price_cops_per_day)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [id, req.user.id, name, description, category || 'whatsapp', coverImage || '', downloadUrl, demoUrl || '', price, rentalPrice]
    });
...
router.put('/listings/:id', protect, async (req, res) => {
  try {
    ...
    let { name, description, coverImage, downloadUrl, demoUrl, priceCOPS, rentalPricePerDay, status } = req.body;
    if (name != null) name = String(name).trim().slice(0, 100) || null;
    if (description != null) description = String(description).trim().slice(0, 2000);
    let priceArg = null;
    if (priceCOPS != null) {
      const p = parseInt(priceCOPS, 10);
      if (!Number.isFinite(p) || p <= 0) return res.status(400).json({ message: 'priceCOPS must be a positive number' });
      priceArg = p;
    }
    let rentalArg = null;
    if (rentalPricePerDay != null) {
      const r = parseInt(rentalPricePerDay, 10);
      rentalArg = (Number.isFinite(r) && r >= 0) ? r : 0;
    }
    await db.execute({
      sql: `UPDATE bot_listings SET name = COALESCE(?, name), description = COALESCE(?, description),
            ...
            WHERE id = ?`,
      args: [name, description, coverImage, downloadUrl, demoUrl, priceArg, rentalArg, status, req.params.id]
    });
...
router.post('/listings/:id/buy', protect, async (req, res) => {
  try {
    ...
    // Insert-first: the unique index on (listing_id, buyer_id) WHERE type='buy'
    // is the real guard here — if two buy requests race in at once, only one
    // insert can win, so only one can go on to charge the buyer.
    const purchaseId = uuidv4();
    const claimed = await db.execute({
      sql: 'INSERT INTO bot_purchases (id, listing_id, buyer_id, type, price_paid) VALUES (?, ?, ?, ?, ?) ON CONFLICT DO NOTHING',
      args: [purchaseId, req.params.id, req.user.id, 'buy', listing.price_cops]
    });
    if (!claimed.info.changes) return res.status(400).json({ message: 'You already own this bot' });

    const deducted = await db.execute({
      sql: 'UPDATE users SET wallet_balance = wallet_balance - ? WHERE id = ? AND wallet_balance >= ?',
      args: [listing.price_cops, req.user.id, listing.price_cops]
    });
    if (!deducted.info.changes) {
      // Roll back the purchase claim since payment failed
      await db.execute({ sql: 'DELETE FROM bot_purchases WHERE id = ?', args: [purchaseId] });
      return res.status(400).json({ message: 'Insufficient COPS balance' });
    }
...
router.post('/listings/:id/rent', protect, async (req, res) => {
  try {
    ...
    const deducted = await db.execute({
      sql: 'UPDATE users SET wallet_balance = wallet_balance - ? WHERE id = ? AND wallet_balance >= ?',
      args: [totalPrice, req.user.id, totalPrice]
    });
    if (!deducted.info.changes) return res.status(400).json({ message: 'Insufficient COPS balance' });
