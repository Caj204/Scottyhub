...
router.post('/wallet/spend', protect, async (req, res) => {
  try {
    ...
    const updated = await db.execute({
      ...
      args: [amount, req.user.id, amount]
    });
    if (!updated.info.changes) return res.status(400).json({ message: 'Insufficient COPS balance' });
    ...
    const result = await db.execute({ sql: 'SELECT wallet_balance FROM users WHERE id = ?', args: [req.user.id] });
    res.json({ message: 'Payment successful', balance: result.rows[0]?.wallet_balance || 0 });
  } catch (err) {
    res.status(500).json({ message: 'Server error' });
  }
});
  ...
  try {
    ...
    if (!isSelf && u.profile_visibility === 'private' && isFollowing.rows.length === 0) {
      return res.json({
        ...
        isFollowing: false, isSelf: false, followerCount: 0, followingCount: 0, postCount: 0, posts: [], badges: [], social: {}
      });
    }
    ...
    const posts = await db.execute({
      sql: `SELECT p.id, p.content, p.media_url, p.media_type, p.channel, p.created_at,
                   (SELECT COUNT(*) FROM likes WHERE post_id = p.id) as like_count,
                   (SELECT COUNT(*) FROM comments WHERE post_id = p.id) as comment_count,
                   EXISTS(SELECT 1 FROM likes WHERE post_id = p.id AND user_id = ?) as liked
            FROM posts p WHERE p.author_id = ? ORDER BY p.created_at DESC LIMIT 40`,
      args: [req.user.id, uid]
    });
