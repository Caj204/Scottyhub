...
router.post('/order', protect, async (req, res) => {
  try {
    ...
    // Deduct wallet atomically — the WHERE clause is the real balance check;
    // this closes a race where two simultaneous orders could both pass a
    // separate SELECT check and overdraw the wallet.
    const deducted = await db.execute({
      sql: 'UPDATE users SET wallet_balance = wallet_balance - ? WHERE id = ? AND wallet_balance >= ?',
      args: [total_cost, req.user.id, total_cost]
    });
    if (!deducted.info.changes) {
      const userResult = await db.execute({ sql: 'SELECT wallet_balance FROM users WHERE id = ?', args: [req.user.id] });
...
router.put('/admin/orders/:id', protect, adminOnly, async (req, res) => {
  try {
    ...
    await db.execute({
      sql: `UPDATE boost_orders SET status = ?, admin_note = ?, updated_at = NOW()::text WHERE id = ?`,
    ...
    if ((status === 'cancelled' || status === 'refunded') && order.status !== 'refunded' && order.status !== 'cancelled') {
      await db.execute({
        ...
        args: [order.total_cost, order.user_id]
      });
      await db.execute({
        sql: 'INSERT INTO wallet_transactions (id, user_id, type, amount, description) VALUES (?, ?, ?, ?, ?)',
