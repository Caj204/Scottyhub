...
const { PLANS, getEffectivePlan } = require('../utils/plans.cjs');
  ...
  try {
    ...
    const updated = await db.execute({
      sql: `UPDATE users SET wallet_balance = wallet_balance - ?, plan = ?, plan_expires_at = ?,
                              is_verified_badge = CASE WHEN ? THEN 1 ELSE is_verified_badge END
            WHERE id = ? AND wallet_balance >= ?`,
      args: [plan.priceCOPS, plan.id, newExpiry.toISOString(), plan.badge ? 1 : 0, req.user.id, plan.priceCOPS]
    });
    if (!updated.info.changes) {
      return res.status(400).json({ message: `Insufficient balance. Need ${plan.priceCOPS} COPS.` });
    }
    await db.execute({
