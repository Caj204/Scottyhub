...
const { protect, adminOnly } = require('../middleware/auth.cjs');
...
router.post('/withdrawals/:id/approve', async (req, res) => {
  try {
    ...
    await db.execute({
      sql: `UPDATE withdrawals SET status = 'approved', resolved_at = NOW()::text, admin_note = ? WHERE id = ?`,
...
router.post('/withdrawals/:id/reject', async (req, res) => {
  try {
    ...
    await db.execute({
      sql: `UPDATE withdrawals SET status = 'rejected', resolved_at = NOW()::text, admin_note = ? WHERE id = ?`,
      args: [req.body.note || '', req.params.id]
    });
...
const NEWS_CONTENT_TYPES = ['news', 'announcement', 'tutorial', 'article', 'video'];

// POST /api/admin/news
router.post('/news', async (req, res) => {
  try {
    let { title, body, icon, color, category, contentType, mediaUrl, mediaType } = req.body;
    if (!title || !body) return res.status(400).json({ message: 'title and body required' });
    title = String(title).trim().slice(0, 200);
    body = String(body).trim().slice(0, 20000);
    if (!title || !body) return res.status(400).json({ message: 'title and body required' });
    if (!NEWS_CONTENT_TYPES.includes(contentType)) contentType = 'news';
    mediaUrl = mediaUrl ? String(mediaUrl).slice(0, 1000) : '';
    mediaType = ['image', 'video'].includes(mediaType) ? mediaType : '';
    const id = uuidv4();
    await db.execute({
      sql: 'INSERT INTO news (id, title, body, icon, color, category, content_type, media_url, media_type, author_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      args: [id, title, body, icon || 'newspaper', color || '#00ffcc', category || 'general', contentType, mediaUrl, mediaType, req.user.id]
    });
    const result = await db.execute({ sql: 'SELECT * FROM news WHERE id = ?', args: [id] });
    res.status(201).json({ message: 'Published!', news: result.rows[0] });
  } catch (err) {
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.put('/news/:id', async (req, res) => {
  try {
    let { title, body, icon, color, category, contentType, mediaUrl, mediaType } = req.body;
    if (title) title = String(title).trim().slice(0, 200);
    if (body) body = String(body).trim().slice(0, 20000);
    if (contentType && !NEWS_CONTENT_TYPES.includes(contentType)) contentType = null;
    mediaUrl = mediaUrl != null ? String(mediaUrl).slice(0, 1000) : null;
    mediaType = ['image', 'video', ''].includes(mediaType) ? mediaType : null;
    await db.execute({
      sql: `UPDATE news SET title=COALESCE(?,title), body=COALESCE(?,body), icon=COALESCE(?,icon),
                   color=COALESCE(?,color), category=COALESCE(?,category), content_type=COALESCE(?,content_type),
                   media_url=COALESCE(?,media_url), media_type=COALESCE(?,media_type) WHERE id=?`,
      args: [title||null, body||null, icon||null, color||null, category||null, contentType||null, mediaUrl, mediaType, req.params.id]
    });
...
router.get('/posts', async (req, res) => {
  try {
    const result = await db.execute({
      sql: `SELECT p.id, p.content, p.media_url, p.media_type, p.pinned, p.created_at,
                   u.id as author_id, u.username as author_username
            FROM posts p JOIN users u ON p.author_id = u.id
            ORDER BY p.pinned DESC, p.created_at DESC LIMIT 100`
    });
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const paidPlans = await db.execute("SELECT COUNT(*) as c FROM users WHERE plan != 'free' AND (plan_expires_at IS NULL OR plan_expires_at > NOW()::text)");
...
// POST /api/admin/announce — pin a message (optionally with an image/video) to the Announcements feed
router.post('/announce', async (req, res) => {
  try {
    let { content, mediaUrl, mediaType } = req.body;
    if (!content) return res.status(400).json({ message: 'Content required' });
    ...
    const id = uuidv4();
    await db.execute({
      sql: 'INSERT INTO posts (id, author_id, content, media_url, media_type, pinned) VALUES (?, ?, ?, ?, ?, 1)',
      args: [id, req.user.id, content, mediaUrl, mediaType]
    });
...
// PUT /api/admin/posts/:id/pin — toggle whether a post shows in Announcements
router.put('/posts/:id/pin', async (req, res) => {
  try {
    const pinned = req.body.pinned ? 1 : 0;
    await db.execute({ sql: 'UPDATE posts SET pinned=? WHERE id=?', args: [pinned, req.params.id] });
    res.json({ message: pinned ? 'Pinned to Announcements' : 'Unpinned' });
  } catch (err) {
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.put('/reports/:id/resolve', async (req, res) => {
  try {
    ...
    await db.execute({
      sql: `UPDATE reports SET status = ?, resolved_at = NOW()::text WHERE id = ?`,
