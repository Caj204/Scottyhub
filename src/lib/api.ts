...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  // A hanging fetch (asleep server, stalled DB connection, dropped
  // connection) would otherwise leave callers awaiting forever with the
  // loading spinner stuck on. Abort and surface a real error instead.
  ...
  try {
    ...
    if (e?.name === 'AbortError') {
      throw new ApiError('The server took too long to respond. It may be waking up from sleep — please try again in a moment.', 0);
    }
